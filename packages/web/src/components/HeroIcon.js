...
export function HeroIcon({ name, size = 20, className = "", title }) {
    ...
    if (idx >= tries.length) {
        ...
        return (_jsx("div", { title: title ?? name, className: `inline-flex items-center justify-center rounded-md bg-slate-700/60 text-slate-200 ${className}`, style: { width: size, height: size, fontSize: size * 0.45 }, children: initials || "?" }));
    }
    const src = `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/dota_react/heroes/${tries[idx]}.png`;
    return (_jsx("img", { src: src, alt: name, title: title ?? name, width: size, height: size, className: `inline-block rounded-md object-cover ${className}`, loading: "lazy", onError: () => setIdx(i => i + 1) }));
}
